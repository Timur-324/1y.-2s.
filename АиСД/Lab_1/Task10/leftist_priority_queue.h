...
protected: 
	...
	virtual void keep_leftist(
		node * n)
	{
		if (get_rank(n->left) < get_rank(n->right)) 
		{
		...
		node* y)
	{
    	...
    	if (_priorities_comparer(x->priority, y->priority) < 0)
		{
        	std::swap(x, y);
    	}
...
public: 
		...
		mergeable_priority_queue const *to_meld_from) const override
	{
		...
		to_meld_into->_root = to_meld_into->merge_trees(to_meld_into->_root, lpq_to_meld_from->_root);
	...
	char *remove_max() override
	{
		...
		char * to_copy = _root->value;
		node *_old_root = _root;

		_old_root->value = nullptr;

		_root = merge_trees(_old_root->left, _old_root->left);

		delete_node_value(_old_root);
		delete _old_root;
